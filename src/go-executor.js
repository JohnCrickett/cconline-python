// Go executor module: manages the Go WASM Web Worker and provides a Promise-based API
...
export function onGoStatus(callback) {
  onStatusCallback = callback;
}

function setupWorker() {
  try {
    worker = new Worker(new URL('./go-worker.js', import.meta.url));
  } catch (err) {
    console.error('Failed to create Go worker:', err);
    if (onStatusCallback) {
      onStatusCallback({
        type: 'load-error',
        error: 'Failed to start Go worker. Your browser may not support Web Workers.',
      });
    }
    return;
  }
  ...
  worker.onerror = function (err) {
    ...
    if (onStatusCallback) {
      onStatusCallback({
        type: 'load-error',
        error: 'Go worker encountered an error. Try refreshing the page.',
      });
    }
  };
}
...
export function initGoWorker() {
  setupWorker();
}

export function retryGoLoad() {
  if (worker) {
    worker.terminate();
    worker = null;
  }
...
export async function runGoCode(code) {
  if (!worker) {
    throw new Error('Go worker not initialized. Call initGoWorker() first.');
  }
  ...
  const executionPromise = new Promise((resolve, reject) => {
    ...
    try {
      ...
      pendingRequests.delete(id);
      reject(new Error('Failed to send code to Go worker: ' + err.message));
    }
  });
...
export function stopGoExecution() {
  if (isExecuting) {
    recreateWorker();
  }
}

export function getIsGoExecuting() {
  return isExecuting;
}
