// JavaScript executor module: manages the JS Web Worker and provides a Promise-based API
...
export function onJsStatus(callback) {
...
function setupWorker() {
  try {
    ...
    if (onStatusCallback) {
      onStatusCallback({
        type: 'load-error',
        error: 'Failed to start JS worker. Your browser may not support Web Workers.',
      });
    }
    return;
  }
  ...
  worker.onerror = function (err) {
    ...
    if (onStatusCallback) {
      onStatusCallback({
        type: 'load-error',
        error: 'JS worker encountered an error. Try refreshing the page.',
      });
    }
  };
}
...
export function initJsWorker() {
  setupWorker();
}

export function retryJsLoad() {
  if (worker) {
    worker.terminate();
    worker = null;
  }
...
export async function runJsCode(code) {
  if (!worker) {
    throw new Error('JS worker not initialized. Call initJsWorker() first.');
  }
  ...
  const executionPromise = new Promise((resolve, reject) => {
    ...
    try {
      ...
      pendingRequests.delete(id);
      reject(new Error('Failed to send code to JS worker: ' + err.message));
    }
  });
...
export function stopJsExecution() {
  if (isExecuting) {
    recreateWorker();
  }
}

export function getIsJsExecuting() {
  return isExecuting;
}
